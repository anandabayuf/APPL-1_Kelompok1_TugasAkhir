...
public class App implements Runnable {
    ...
    void loginIO(){
        ...
        final String template_1 = "\n\nChoose from the Following options:\n\n" +
                                ...
                                "- For viewing list of fines from the system, press 9\n" +
                                "- For viewing list of issued resources from the system, press 10\n" +
                                "- For viewing list of pending request from the system, press 11\n" +
                                "- For logging out of the system, press 12";
            ...
            if(user.getType() == Type.ADMIN) {         
                ...
                switch (this.scanner.nextInt()) {
                    ...
                    case 8:
                        ...
                        break;
                    case 9:
                        viewListOfFinesIO((Admin)user);
                        break;
                    case 10:
                        viewListOfIssuedResources((Admin)user);
                        break;
                    case 11:
                        viewListOfPendingRequest((Admin)user);
                        break;
                    case 12:
    ...
    void viewListOfFinesIO(Admin admin){
        List<User> users = library.getUserManager().getUsers();
        if(!users.isEmpty()){
            int usersNum = 1;
            for(User user : users){
                if(user.getType() != User.Type.ADMIN){
                    System.out.println(usersNum + "-" + user.getUsername());
    ...
    void viewListOfIssuedResources(Admin admin){
        List<User> users = library.getUserManager().getUsers();
        if(!users.isEmpty()){
            int usersNum = 1;
            for(User user : users){
    ...
    void viewListOfPendingRequest(Admin admin){
        List<User> users = library.getUserManager().getUsers();
        if(!users.isEmpty()){
            int usersNum = 1;
            for(User user : users){
                if(user.getType() != User.Type.ADMIN){
                    System.out.println("\n"+usersNum + "-" + user.getUsername());
